...
* **Теоретичне обґрунтування:** Буферний розчин складається зі слабкої кислоти ($CH_3COOH$) та її солі ($CH_3COONa$). Для обчислення його водневого показника використовується рівняння Гендерсона-Хассельбаха:
  ...
  *(Примітка: оскільки кислота і сіль містяться в одному загальному об'ємі розчину, співвідношення молярних концентрацій C(солі) / C(кислоти) дорівнює співвідношенню кількостей речовин n(солі) / n(кислоти)).*
...
* **Крок 2: Знаходження кількості речовини ($n$) компонентів буфера.**
  * Для кислоти: $n(\text{кислоти}) = C(\text{кислоти}) \times V(\text{кислоти}) = 0.1 \text{ моль/л} \times 0.1 \text{ л} = 0.01 \text{ моль}$
  * Для солі: $n(\text{солі}) = C(\text{солі}) \times V(\text{солі}) = 0.2 \text{ моль/л} \times 0.2 \text{ л} = 0.04 \text{ моль}$

* **Крок 3: Підстановка даних у рівняння.**
  
  $$pH = 4.75 + \lg\left(\frac{0.04}{0.01}\right) = 4.75 + \lg(4)$$

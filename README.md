...
* **Теоретичне обґрунтування:** Буферний розчин складається зі слабкої кислоти ($CH_3COOH$) та її солі ($CH_3COONa$). Для обчислення його водневого показника використовується рівняння Гендерсона-Хассельбаха:
  ...
  *(Примітка: оскільки кислота і сіль містяться в одному загальному об'ємі розчину, співвідношення молярних концентрацій $\frac{C_{\text{солі}}}{C_{\text{кислоти}}}$ дорівнює співвідношенню кількостей речовин $\frac{n_{\text{солі}}}{n_{\text{кислоти}}}$).*

* **Крок 1: Обчислення показника константи кислотності ($pK_a$).**
  $$pK_a = -\lg(K_a) = -\lg(1.76 \times 10^{-5})$$
  Використовуючи властивості логарифмів ($-\lg(a \times 10^{-b}) = b - \lg(a)$):
...
* **Математичний зміст:** Маємо рівняння прямої лінії у вигляді $y = mx + b$, де $y$ — оптична густина ($A$), $x$ — шукана концентрація ($C$), $m = 0.125$ — кутовий коефіцієнт (нахил прямої), а $b = 0.002$ — вільний член (фонове поглинання або зсув нуля приладу).

* **Крок 1: Підстановка виміряного значення в рівняння регресії.**
  $$0.440 = 0.125 \cdot C + 0.002$$

* **Крок 2: Алгебраїчне вираження невідомого $C$.**
  Переносимо вільний член у ліву частину з протилежним знаком:
  $$0.125 \cdot C = 0.440 - 0.002$$
  $$0.125 \cdot C = 0.438$$

* **Крок 3: Обчислення кінцевого результату.**
  $$C = \frac{0.438}{0.125}$$
  *(Підказка: поділити число на $0.125$ — це те саме, що помножити його на $8$)*:
  $$C = 0.438 \times 8 = 3.504 \approx 3.50 \text{ мг/л}$$

* **Відповідь:** Концентрація заліза становить $3.50 \text{ мг/л}$.
...
![Калібрувальний графік для марганцю](images/1.png)
...
![Калібрувальний графік для нітратів](images/2.png)
...
![Калібрувальний графік для білка](images/3.png)
...
![Калібрувальний графік для рибофлавіну](images/4.png)
...
![Калібрувальний графік для pH](images/5.png)
